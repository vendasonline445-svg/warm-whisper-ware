    ...
    <div className="min-h-screen bg-white pb-[72px]">
      ...
      {shareOpen && (
        <div className="fixed inset-0 z-[70] flex items-end justify-center">
          ...
          <div className="relative w-full max-w-lg bg-background rounded-t-3xl animate-in slide-in-from-bottom duration-300">
            {/* Drag handle */}
            <div className="flex justify-center pt-3 pb-1">
            ...
            <div className="flex items-center justify-between px-4 pb-3">
              <div className="w-6" />
              <h3 className="font-semibold text-[15px]">Enviar para</h3>
              <button onClick={() => setShareOpen(false)} className="p-1"><X className="h-4 w-4 text-muted-foreground" /></button>
            </div>
            <div className="flex items-start justify-between px-6 pb-6 pt-1">
              {[
                { name: "Copiar\nlink", icon: <Link className="h-5 w-5 text-white" />, bg: "bg-muted-foreground", action: () => { navigator.clipboard.writeText(window.location.href); setCouponCopied(true); setTimeout(() => setCouponCopied(false), 2000); } },
                { name: "WhatsApp", icon: <svg viewBox="0 0 24 24" className="h-5 w-5 fill-white"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347z"/><path d="M12 0C5.373 0 0 5.373 0 12c0 2.127.555 4.126 1.526 5.86L0 24l6.335-1.652A11.95 11.95 0 0012 24c6.627 0 12-5.373 12-12S18.627 0 12 0zm0 21.75c-2.006 0-3.885-.588-5.47-1.597l-.393-.237-3.762.982.999-3.648-.26-.414A9.72 9.72 0 012.25 12 9.75 9.75 0 0112 2.25 9.75 9.75 0 0121.75 12 9.75 9.75 0 0112 21.75z"/></svg>, bg: "bg-[#25D366]", action: () => window.open(`https://wa.me/?text=${encodeURIComponent('Olha essa mesa dobrável incrível! ' + window.location.href)}`, '_blank') },
                { name: "Instagram\nDirect", icon: <svg viewBox="0 0 24 24" className="h-5 w-5 fill-white"><path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zM12 0C8.741 0 8.333.014 7.053.072 2.695.272.273 2.69.073 7.052.014 8.333 0 8.741 0 12c0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98C8.333 23.986 8.741 24 12 24c3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98C15.668.014 15.259 0 12 0zm0 5.838a6.162 6.162 0 100 12.324 6.162 6.162 0 000-12.324zM12 16a4 4 0 110-8 4 4 0 010 8zm6.406-11.845a1.44 1.44 0 100 2.881 1.44 1.44 0 000-2.881z"/></svg>, bg: "bg-gradient-to-br from-[#f09433] via-[#dc2743] to-[#bc1888]", action: () => window.open(`https://www.instagram.com/`, '_blank') },
                { name: "Facebook", icon: <svg viewBox="0 0 24 24" className="h-5 w-5 fill-white"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg>, bg: "bg-[#1877F2]", action: () => window.open(`https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(window.location.href)}`, '_blank') },
                { name: "SMS", icon: <svg viewBox="0 0 24 24" className="h-5 w-5 fill-white"><path d="M20 2H4c-1.1 0-2 .9-2 2v18l4-4h14c1.1 0 2-.9 2-2V4c0-1.1-.9-2-2-2zm0 14H5.17L4 17.17V4h16v12z"/><path d="M7 9h2v2H7zm4 0h2v2h-2zm4 0h2v2h-2z"/></svg>, bg: "bg-[#34B7F1]", action: () => window.open(`sms:?body=${encodeURIComponent('Olha essa mesa dobrável incrível! ' + window.location.href)}`) },
                { name: "Telegram", icon: <svg viewBox="0 0 24 24" className="h-5 w-5 fill-white"><path d="M11.944 0A12 12 0 000 12a12 12 0 0012 12 12 12 0 0012-12A12 12 0 0012 0h-.056zm4.962 7.224c.1-.002.321.023.465.14a.506.506 0 01.171.325c.016.093.036.306.02.472-.18 1.898-.962 6.502-1.36 8.627-.168.9-.499 1.201-.82 1.23-.696.065-1.225-.46-1.9-.902-1.056-.693-1.653-1.124-2.678-1.8-1.185-.78-.417-1.21.258-1.91.177-.184 3.247-2.977 3.307-3.23.007-.032.014-.15-.056-.212s-.174-.041-.249-.024c-.106.024-1.793 1.14-5.061 3.345-.479.33-.913.49-1.302.48-.428-.008-1.252-.241-1.865-.44-.752-.245-1.349-.374-1.297-.789.027-.216.325-.437.893-.663 3.498-1.524 5.83-2.529 6.998-3.014 3.332-1.386 4.025-1.627 4.476-1.635z"/></svg>, bg: "bg-[#0088cc]", action: () => window.open(`https://t.me/share/url?url=${encodeURIComponent(window.location.href)}&text=${encodeURIComponent('Olha essa mesa dobrável incrível!')}`, '_blank') },
              ].map((item) => (
                <button key={item.name} onClick={() => { item.action(); }} className="flex flex-col items-center gap-1.5 flex-1 min-w-0">
                  <div className={`h-12 w-12 rounded-full ${item.bg} flex items-center justify-center shadow-sm`}>
                    {item.icon}
                  </div>
                  <span className="text-[10px] text-muted-foreground text-center leading-tight whitespace-pre-line">{item.name}</span>
                </button>
              ))}
            </div>
            {couponCopied && <p className="text-center text-xs text-cta font-medium pb-3 -mt-2">Link copiado!</p>}
          </div>
        </div>
      )}
